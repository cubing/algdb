import { getPusher } from "../tier1/pusher";

import sharedHelper from "../tier0/shared";

import { mysqlHelper, sequelizeDataTypes } from "jomql";
...
export function handleWebhook(req, res) {
  req.body.events.forEach((event) => {
    if (event.name === "channel_vacated") {
      ...
      mysqlHelper.executeDBQuery(
        "DELETE FROM jqlSubscription WHERE channel = :channel",
        {
